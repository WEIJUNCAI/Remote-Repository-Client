...
size_t HttpMessage::findAttribute(const Name& name) const
{
 ...
 */
Value HttpMessage::findValue(const Name& name) const
{
  Value val = "";
...
HttpMessage HttpMessage::parseMessage(const std::string& msgStr)
{
	HttpMessage msg;
	size_t payloadLenght = 0;
	std::istringstream iss(msgStr);
	// read message attributes
	while (true)
	{
		char buffer[200];
		iss.getline(buffer, 200);
		std::string attribString(buffer);
		if (attribString.size() > 1)
		{
			HttpMessage::Attribute attrib = HttpMessage::parseAttribute(attribString);
			if (attrib.first == "Content Length")
				payloadLenght = Converter<size_t>::toValue(attrib.second);
			msg.addAttribute(attrib);
		}
		else
		{
			if (payloadLenght != 0) // read message body
			{
				std::string bodyStr;
				char buffer[200];
				iss.read(buffer, payloadLenght);
				buffer[payloadLenght] = '\0';
				bodyStr += std::string(buffer);
				msg.addBody(bodyStr);
			}
			break;
		}
...
std::string HttpMessage::headerString() const
{
  ...
  header += term_; // append an empty line to indicate the end of header
  return header;
}
